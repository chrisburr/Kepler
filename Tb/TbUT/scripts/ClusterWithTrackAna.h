...
public :
  ...
  ClusterWithTrackAna(TString fi1le, TString board, TString sector, TString bias);
...
// used to generate this class and read the Tree.
  ...
  TTree* tree = 0;
   ...
   TString badStripFileName = "./BadStrips/BadStrips_"+m_board+".dat";
   ...
   if(!badStripFile) { // file couldn't be opened
     ...
     while ( !badStripFile.eof() ) {
       ...
       badStrips[iChan] = 1;
       nbadStrips++;
       cout << "BadStrip = " << nbadStrips << ", Chan# " << iChan << endl;
     }
   }
}

ClusterWithTrackAna::ClusterWithTrackAna(TString file, TString board, TString sector, TString bias){
  //ClusterWithTrackAna::ClusterWithTrackAna(TTree *tree){
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
  ...
  outdir.ReplaceAll("~",home);
  status = mkdir(outdir, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  cout << "Creating output directory: " << outdir << ", status = " << status << endl;
